...
- a **background bed**: the full recording with every emerging event —
  bells, voices, doors — capped down into the continuous background;
- a **characteristic excerpt**: the one minute (or any duration) of the
  *original, untouched* audio that best summarizes what the place sounds
...
compact, artifact-free *summary* of the space — a soundscape thumbnail.
...
**background surface** — the median magnitude in 10 s blocks, followed by
a running *minimum* over ±60 s of blocks. The minimum-statistics step is
...
quiet iterative keynotes survive too — a tower clock's ticking keeps
...
everywhere, and — the best sanity check — a steady-state soundscape like
...
session median), weighted at 20 dB. The winner is exported **bit-exact
from the original take** — no processing whatsoever — together with
...
post-chime minute a human picked for clock-tick analysis — ticking, room
tone and distant city, at 0.5 dB spectral distance from the background
profile.
...
- Like `rhythm` and `carillon`, the command processes the **first take**
  ...
  scene) render whichever sorts first — point at single-file folders for
  per-device beds.
...
- On takes much shorter than a minute the background estimate collapses
  to a single block, so "background" is only weakly defined — expect
  gentler foreground suppression there.
