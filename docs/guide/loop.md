...
**prototype** of the place. Where the [characteristic
excerpt](background-render.md) seeks the *background* — the minute when
nothing in particular happens — the loop seeks the **typical**: the
...
`analysis/loop_<take>_<dur>s.wav` (24-bit, native channel count — 4-ch
...
3. **event-density typicality** — |window eventful-fraction − take
   eventful-fraction|, weighted ×20. Typicality, not calm: a window with
   the *representative* amount of activity beats both dead air and a
   burst of events;
...
player — no DAW crossfade needed. `loop.json` reports `seam_db`, the
