...
mechanical-synchronisation observation.
...
onset-picking threshold and `min_gap_floor` guards the intra-cycle
separation, and both stabilise picking when the strike ODF is noisy.
`rhythm.json` records the
...
- Sources sharing most of their partials (unison bells) will merge into one
  ...
  *both* sides, so use `rise_spectrum` on each to decide which one is the real
