...
Keep it that way — someone analysing a field recording should not have to install a computer-vision
stack, and someone analysing accelerometer data should not have to install an audio one.
...
signals — a tap that registers in the accelerometer *and* the microphone — not on file timestamps.
...
MGT reads the video that ambiscape's session folder usually sits beside — 360 recordings, action
cameras, room video. For GoPro `.360` and similar, MGT's `flatten_gopro360` handles the projection.
...
question belongs to, and say in your analysis which implementation produced a number — two
