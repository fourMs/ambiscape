...
def draft_annotations(F: dict, folder: str | Path,
    ...
    for i0, i1 in _gap_split(F["t"]):
        ...
        for a, b, lvl in _regimes(tf[m], fast[m], dt):
            obj = {
                ...
                "kind": "keynote", "mass": "TODO", "facture": "TODO",
                "label": "TODO — listen and name this ground "
                         f"(median {lvl:.0f} dBFS)",
                "spans": [[_fmt(a), _fmt(b)]],
            }
    ...
    doc = {
        "_instructions": (
            "DRAFT generated by ambiscape. For each object: listen, then set "
            "kind (keynote|signal|soundmark|figure), mass (tonic|"
            "tonic-complex|complex|noise) and facture (impulse|iteration|"
            "sustained|unlimited); rename it; split 'events (unclassified)' "
